		...
		<ul class="nav">
			<li class="dropdown">
				<a class="dropdown-toggle" data-toggle="dropdown" href="#cm_submenu_5">
					<img class="profilepic" src="'.$CFG->wwwroot.'/user/pix.php?file=/'.$USER->id.'/f1.jpg" width="80px" height="80px" title="'.$USER->firstname.' '.$USER->lastname.'" alt="'.$USER->firstname.' '.$USER->lastname.'" />
					<div id="profile-toggler">'.$USER->firstname.'<img id="profile-toggle-up" src="'.$OUTPUT->pix_url('dropdown-up', 'theme').'">
						<img id="profile-toggle-down" src="'.$OUTPUT->pix_url('dropdown-down', 'theme').'">
					</div>
				</a>
				...
				if ($hasemailurl) {
					echo '<li>';
						echo '<a href="'.$PAGE->theme->settings->emailurl.'">';
							...
							echo get_string('email','theme_dspcentral');
						echo '</a>';
					echo '</li>';
				}
